...
if (isset($_SESSION['ID_user'])) {
    ...
    // var_dump($user_films);
    $film_count_request = $con->prepare("SELECT ID_film FROM user_fav WHERE ID_user = $ID_user");
    $film_count_request->execute();
    ...
    if (isset($_GET['genre'])) {
        ...
        foreach ($get_genre_array as $ID_genre) {
            $request = $con->prepare("SELECT ID_film FROM film_genre WHERE ID_genre = $ID_genre");
            $request->execute();
    ...
    if (isset($filters['sort'])) {
        if ($filters['sort'] == 'a-z') {
            $order_name = 'film_name ASC';
        };
        if ($filters['sort'] == 'z-a') {
            $order_name = 'film_name DESC';
        };
        if ($filters['sort'] == 'rand') {
            $order_name = 'rand()';
        };
        if ($filters['sort'] == 'asc') {
            $order_name = 'ID_film ASC';
        };
        if ($filters['sort'] == 'desc') {
            $order_name = 'ID_film DESC';
        };
        if ($filters['sort'] == 'grade') {
            $order_name = 'film_grade DESC';
        };
        if ($filters['sort'] == 'date') {
            $order_name = 'film_date DESC';
        };
        if ($filters['sort'] == 'fav') {
            $order_name = 'likes DESC';
        };
    ...
    if (empty($_SESSION['filters'])) {
        $url = '/content/favoris.php?';
    } else {
        $url = '/content/favoris.php?' . $_SESSION['filters'] . '&';
    }
    // var_dump($url);
...
<div class="flex justify-center my-4  ">
    <nav aria-label="Page navigation example">
        ...
        if ($pagination_number > $page_number - 3 && $pagination_number < $page_number + 3) { ?>
          ...
          <li>
            <a href="/content/catalogue.php?page=<?= $page_count ?>" class="block px-3 py-2 leading-tight text-gray-400 hover:text-main-light hover:font-bold ">
...
<?php }
    ...
    <!-- CATALOGUE -->
        <div class="grid gap-4 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:-grid-cols-5 [&_img]:w-full [&_img]:h-full object-cover ">
            ...
            <a href="/content/film.php?page=<?= $ID ?>" class="h-full cursor-pointer overflow-hidden">
                            <!-- OVERLAY!!! -->
                            <div class="hidden lg:block z-50 absolute w-full h-full bg-main-dark bg-opacity-80 opacity-0 group-hover:opacity-100 group p-4">
                                <div class="relative w-full h-full flex flex-col justify-between">
                                    ...
                                    <?php
                                    if (isset($_SESSION['ID_user'])) {
                                        isFilmFav($ID, $_SESSION['ID_user'], $likes);
                                    } else {
                                        ShowFakeFav($likes);
                                    }
                                    ?>
                                ...
                                <div class="h-96 md:h-[400px]  mx-auto w-fit relative">
                                    ...
                                    <img src="/upload/film/<?= $photo ?>" class="h-full">
                                </div>
                            </a>
            </div>
            <?php } ?>
...
<?php include ('../include/general/footer.php');
    ...
    echo '<a href="/index.php" class="text-main-light">Retour à l\'accueil';
} ?>
