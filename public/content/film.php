...
<head>
    ...
    <link rel="icon" type="image/x-icon" href="/img/popcorn.png">
    <script>
        tailwind.config = {
            theme: {
    extend: {
...
<body class="bg-main-dark text-gray-100 bg-cover bg-center bg-fixed bg-no-repeat " style="background-image:url(/upload/film/<?= $film['film_background'] ?>)">
...
if (isset($_SESSION['ID_user'])) {
    isFilmFav($ID_film, $_SESSION['ID_user'], $film['likes']);
} else {
    ShowFakeFav($film['likes']);
}
?>
    </div>
</div>
<!-- GRILLE -->
    <div class="block md:grid md:grid-cols-2 lg:gap-8  ">
        <div class="p-2 sm:p-4 lg:p-8">
            <img src="/upload/film/<?= $film['film_photo'] ?>" class=" w-full sm:w-[75vw] mx-auto md:w-full" alt="">
        </div>
        <div class="[&>*]:my-4 my-8 relative">
            ...
            <div class="px-8 font-bold text-sm sm:text-base lg:text-xl flex flex-wrap gap-y-4 gap-x-2"><p class="underline ">Genres : </p>
                ...
                while ($genres = $request->fetch()) {
                    ...
                    while ($genre = $genre_info->fetch()) { ?>
                        <a href="/" class="text-main-light hover:text-main-hover font-normal"><?= $genre['genre_name'] ?></a>
                <?php }
                } ?>
            </div>
            <div class="px-8 font-bold text-sm sm:text-base lg:text-xl flex flex-wrap gap-y-4 gap-x-3"><p class="underline ">Acteurs principaux : </p>
                ...
                while ($actors = $request->fetch()) {
                    ...
                    <a href="/" class="text-main-light relative hover:text-main-hover hover:underline font-normal group">
                        <!-- overlay acteur -->
                        <div class="opacity-0 pointer-events-none group-hover:opacity-100 -translate-y-20 group-hover:-translate-y-0  group-hover:linear duration-500  absolute bottom-10 w-24 aspect-square object-cover ">
                            <img class="w-full h-auto" src="/upload/actor/<?= $actor['actor_photo'] ?>">
                        </div>
        ...
        while ($realisators = $request->fetch()) {
            ...
            while ($realisator = $realisator_info->fetch()) { ?>
                    <a href="/" class="text-main-light relative hover:text-main-hover hover:underline font-normal group">
                        <!-- overlay réalisateur -->
                        <div class="opacity-0 pointer-events-none group-hover:opacity-100 -translate-y-20 group-hover:-translate-y-0  group-hover:linear duration-500 absolute bottom-10 w-24 aspect-square object-cover">
                            <img class="w-full h-auto" src="/upload/realisator/<?= $realisator['realisator_photo'] ?>">
                        </div>
            ...
            while ($scenarist = $scenarist_info->fetch()) { ?>
                    <a href="/" class="text-main-light relative hover:text-main-hover hover:underline font-normal group">
                        ...
                        <div class="opacity-0 pointer-events-none group-hover:opacity-100 translate-y-20 group-hover:translate-y-0  group-hover:linear duration-500 absolute top-10 w-24 aspect-square object-cover">
                            <img class="w-full h-auto" src="/upload/scenarist/<?= $scenarist['scenarist_photo'] ?>">
                        </div>
...
<?php $comment_request = $con->prepare('SELECT * FROM comment WHERE ID_film = ?');
$comment_request->execute([$film['ID_film']]);
...
<section class="bg-transparent py-8 lg:py-16">
  <div class="max-w-2xl mx-auto px-4">
    ...
    <form class="mb-6" method="post" action="/traitements/add/add_comment.php">
    ...
    <article class="reveal p-6 text-base mb-6 border-t  border-main-light bg-main-dark">
        <footer class="flex justify-between items-center mb-2">
            <div class="flex items-center">
                <p class="inline-flex items-center capitalize text-sm  text-gray-50"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="mr-3 w-8 h-8">
                    ...
                    </svg><?php $ID_user = $comment['ID_user'];
        $comment_pseudo_request = $con->prepare("SELECT user_pseudo FROM user WHERE ID_user = $ID_user");
        $comment_pseudo_request->execute();
        $comment_pseudo = $comment_pseudo_request->fetch(); ?><?= $comment_pseudo[0] ?></p>
            ...
            class="hidden z-10 w-fit  rounded divide-y  shadow bg-main-dark divide-gray-600">
                ...
                <ul class="py-1  text-sm  text-gray-200"
                    ...
                    <?php } ?>
            <form method="post" action="/traitements/manage_comment.php">
        ...
        <form id="new_comment<?= $comment['ID_comment'] ?>" class="hidden" method="post" action="/traitements/manage_comment.php">
            <input class="hidden" name="ID_comment" value="<?= $comment['ID_comment'] ?>">
        ...
        <div class="relative h-96 w-[80%] mx-auto overflow-hidden rounded-lg  md:h-[400px]">
            ...
            <div class="hidden duration-3000 ease-in-out h-full" data-carousel-item>
                ...
                <?php for ($i = 0; $i < 5; $i++) { ?>
                            ...
                            <a href="/content/film.php?page=<?= $similar_films[$i]['ID_film'] ?>" class="h-full cursor-pointer overflow-hidden">
                            ...
                            <?php
                            if (isset($_SESSION['ID_user'])) {
                                isFilmFav($similar_films[$i]['ID_film'], $_SESSION['ID_user'], $similar_films[$i]['likes']);
                            } else {
                                ShowFakeFav($similar_films[$i]['likes']);
                            }
                            ?>
                                    <div>
                                ...
                                <div class="relative w-fit h-96 md:h-[400px] mx-auto">
                                    ...
                                    <img src="/upload/film/<?= $similar_films[$i]['film_photo'] ?>" class="h-full">
                                </div>
                            </a>
                        </div> <?php } ?>
                </div>
