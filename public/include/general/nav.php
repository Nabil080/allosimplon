<nav id="navbar" class="text-main-light bg-main-dark fixed top-0 w-full z-40 ease-out duration-300">
    <div id="nav-contain" class="flex p-4 md:gap-4  w-full h-20 md:px-[10%]">
        <img src="/upload/site/popcorn.png" class="basis-auto" alt="logo"><a class="self-center" href="/index.php">
            <span class="text-gray-100 uppercase self-center text-base md:text-xl ">SimplonFilm</span></a>
        <form action="/traitements/search.php" method="get" class="grow flex relative">
            ...
            <div id="searchResult" style="display:none;"  class="overflow-scroll max-h-screen absolute top-12 flex flex-col w-full bg-main-dark gap-2 [&>*]:px-4 [&>*]:py-2 border border-t-transparent ">
                ...
                foreach ($all_film as $film_search) {
                    $url = '/content/catalogue.php?page=1&search=' . $film_search['film_name'];
                    ?>
    ...
    <form action="/traitements/search.php" method="get" class="grow flex relative md:hidden">
    ...
    <button><a class="hover:text-gray-300" href="/index.php">Accueil</a>
        <button data-dropdown-delay="100" data-dropdown-trigger="hover" data-dropdown-toggle="CatalogueDropdown" class="hidden md:block"><a class="hover:text-gray-300 no-underline" href="/content/catalogue.php?page=1">Catalogue<i class="fa-solid fa-chevron-down pl-1 text-lg"></i></a></button>
        <button class="block md:hidden"><a class="hover:text-gray-300 no-underline" href="/content/catalogue.php?page=1">Catalogue</a></button>
            <div id="CatalogueDropdown" class="hidden bg-main-dark border-t-main-default px-[10%]"><ul class="flex gap-4 flex-wrap mb-4 justify-center [&_li]:text-xl [&_li]:text-main-light [&_li]:underline">
                <a href="/content/catalogue?page=1&sort=a-z"><li class="hover:text-main-hover">Par ordre alphabétique</li></a>
                <a href="/content/catalogue?page=1&sort=fav"><li class="hover:text-main-hover">Par nombre de likes</li></a>
                <a href="/content/catalogue?page=1&sort=grade"><li class="hover:text-main-hover">Par note IMDb</li></a>
                <a href="/content/catalogue?page=1&sort=date"><li class="hover:text-main-hover">Par ordre de sortie</li></a>
                <a href="/content/catalogue?page=1&sort=asc"><li class="hover:text-main-hover">Par ordre d'ajout</li></a>
                <a href="/content/catalogue?page=1&sort=rand"><li class="hover:text-main-hover">Par ordre aléatoire</li></a>
            </ul></div>
        <button><a class="hover:text-gray-300" href="/content/actor.php">Acteurs</a></button>
        <button class=" no-underline"  data-dropdown-delay="100" data-dropdown-trigger="hover" data-dropdown-toggle="GenresDropdown">Genres<i class="fa-solid fa-chevron-down pl-1 text-lg"> </i></button>
            ...
            <?php $genre_list_request = $con->prepare('SELECT * FROM genre');
            $genre_list_request->execute();
            while ($genre_list = $genre_list_request->fetch()) { ?>
                <a href="/content/catalogue.php?page=1&genre[]=<?= $genre_list['ID_genre'] ?>"class=""><li class="text-xl text-main-light hover:text-main-hover  underline "> <?= $genre_list['genre_name'] ?> </li></a>
            <?php } ?>
            </ul></div>
...
<button id="message_box" class="duration-500 ease-linear fixed top-[10%] right-16 px-5 py-2 md:px-10 md:py-5 bg-main-light bg-opacity-80 z-50 animate-bounce rounded-lg hover:bg-main-hover">
    <?php
    if ($_GET['message'] == 'no_form') {
        echo 'Venez depuis un formulaire !';
    }
    if ($_GET['message'] == 'missing_element') {
        echo 'Un élément est manquant !';
    }

    if ($_GET['message'] == 'mail_invalid') {
        echo 'E-mail invalide!';
    }
    if ($_GET['message'] == 'mail_existing') {
        echo 'E-mail déjà existant!';
    }
    if ($_GET['message'] == 'size_verif_password') {
        echo 'Les mots de passes ne correspondent pas ou sont trop longs !';
    }
    if ($_GET['message'] == 'size_pseudo') {
        echo 'Le pseudo est trop long ou invalide !';
    }
    if ($_GET['message'] == 'inscrit') {
        echo 'Inscrit avec succès !';
    }

    if ($_GET['message'] == 'wrong_mail') {
        echo 'E-mail incorrect !';
    }
    if ($_GET['message'] == 'wrong_password') {
        echo 'Mot-de-passe incorrect !';
    }
    if ($_GET['message'] == 'connected') {
        echo 'Connecté avec succès !';
    }
    if ($_GET['message'] == 'not_connected') {
        echo "Vous n'êtes pas connecté !";
    }

    if ($_GET['message'] == 'update_password') {
        echo 'Mot-de-passe modifié !';
    }
    if ($_GET['message'] == 'update_mail') {
        echo 'E-mail modifié !';
    }
    if ($_GET['message'] == 'mail_verif_error') {
        echo 'Les e-mails ne correspondent pas !';
    }
    if ($_GET['message'] == 'update_pseudo') {
        echo 'Pseudo modifié !';
    }

    if ($_GET['message'] == 'add_comment') {
        echo 'Commentaire ajouté !';
    }
    if ($_GET['message'] == 'update_comment') {
        echo 'Commentaire modifié !';
    }
    if ($_GET['message'] == 'delete_comment') {
        echo 'Commentaire supprimé !';
    }
    if ($_GET['message'] == 'report_comment') {
        echo 'Commentaire signalé !';
    }

    if ($_GET['message'] == 'add_note') {
        echo 'Note ajouté !';
    }
    if ($_GET['message'] == 'update_note') {
        echo 'Note modifié !';
    }

    if ($_GET['message'] == 'add_fav') {
        echo 'Ajouté aux favoris !';
    }
    if ($_GET['message'] == 'delete_fav') {
        echo 'Supprimé des favoris !';
    }

    if ($_GET['message'] == 'add_film') {
        echo 'Film ajouté !';
    }
    if ($_GET['message'] == 'update_film') {
        echo 'Film modifié !';
    }
    if ($_GET['message'] == 'delete_film') {
        echo 'Film supprimé !';
    }
    ...
    if ($_GET['message'] == 'add_actor') {
        echo 'Acteur ajouté !';
    }
    if ($_GET['message'] == 'update_actor') {
        echo 'Acteur modifié !';
    }
    if ($_GET['message'] == 'delete_actor') {
        echo 'Acteur supprimé !';
    }

    if ($_GET['message'] == 'add_realisator') {
        echo 'Réalisateur ajouté !';
    }
    if ($_GET['message'] == 'update_realisator') {
        echo 'Réalisateur modifié !';
    }
    if ($_GET['message'] == 'delete_realisator') {
        echo 'Réalisateur supprimé !';
    }

    if ($_GET['message'] == 'add_scenarist') {
        echo 'Scénariste ajouté !';
    }
    if ($_GET['message'] == 'update_scenarist') {
        echo 'Scénariste modifié !';
    }
    if ($_GET['message'] == 'delete_scenarist') {
        echo 'Scénariste supprimé !';
    }

    if ($_GET['message'] == 'add_genre') {
        echo 'Genre ajouté !';
    }
    if ($_GET['message'] == 'update_genre') {
        echo 'Genre modifié !';
    }
    if ($_GET['message'] == 'delete_genre') {
        echo 'Genre supprimé !';
    }

    if ($_GET['message'] == 'size_error') {
        echo 'Fichier trop volumnieux !';
    }
    if ($_GET['message'] == 'format_error') {
        echo 'Format du fichier incorrect !';
    }
    if ($_GET['message'] == 'move_file') {
        echo 'Le fichier a été déplace dans le serveur !';
    }
    if ($_GET['message'] == 'move_file_error') {
        echo "Le fichier n'a pas été déplacé dans le serveur !";
    }
    if ($_GET['message'] == 'file_error') {
        echo 'Erreur avec le fichier !';
    }
}
    ...
    <div class="relative w-full h-fit max-w-md md:h-auto">
        ...
        <div class="relative bg-main-dark border-main-light border-2 rounded-lg shadow text-gray-100">
            ...
            <div id="co" class="px-6 py-6 lg:px-8">
                ...
                <form class="space-y-6" action="/traitements/connexion/login.php" method="post">
                    <div>
            ...
            <div id="paco" class="hidden px-6 py-6 lg:px-8">
                ...
                <form class="space-y-6" action="/traitements/connexion/sign.php" method="post">
                    <div>
        ...
        <div class="relative bg-main-dark border-main-light border-2 rounded-lg shadow text-gray-100">
            ...
            <div id="profil_card" class="px-6 py-6 lg:px-8">
                ...
                <div class="flex justify-center">
                    <button type="button" class=" text-gray-100 bg-main-light hover:bg-main-hover focus:ring-main-light focus:ring-offset-main-light font-medium rounded-lg  px-5 py-2.5 mr-2 mb-2 focus:outline-none"><a href="/content/favoris.php">Voir vos favoris</a></button>
                    <?php if ($_SESSION['ID_role'] == 1) { ?><button type="button" class=" text-gray-100 bg-main-light hover:bg-main-hover focus:ring-main-light focus:ring-offset-main-light font-medium rounded-lg  px-5 py-2.5 mr-2 mb-2 focus:outline-none"><a href="/content/crud.php">Interface admin</a></button><?php } ?>
                </div>
                <div class="flex justify-end w-full">
                    <a class="text-sm text-main-light ml-4" href="/traitements/connexion/logout.php">Se déconnecter</a>
                </div>
            </div>
            ...
            <div id="pseudo_form" class="px-6 py-6 lg:px-8 hidden">
                ...
                <form class="space-y-6" action="/traitements/modify/modify_pseudo.php" method="post">
                    <div>
            ...
            <div id="mail_form" class="px-6 py-6 lg:px-8 hidden">
                ...
                <form class="space-y-6" action="/traitements/modify/modify_mail.php" method="post">
                    <div>
            ...
            <div id="pass_form" class="px-6 py-6 lg:px-8 hidden">
                ...
                <form class="space-y-6" action="/traitements/modify/modify_password.php" method="post">
                    <div>
